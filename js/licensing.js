...
async function save(){
 ...
 try{const user=api.auth.currentUser,p=profile()||{},payloadRows=rows.map(row=>({department:clean(row.department),projectName:clean(row.projectName),unitValue:row.unitValue===null?0:num(row.unitValue)}));await fs.setDoc(fs.doc(api.db,'licensing_budget','fy2027'),{fiscalYear:YEAR,currency:'USD',rows:payloadRows,total:total(),updatedBy:user.uid,updatedByEmail:clean(user.email||p.email).toLowerCase(),updatedAt:fs.serverTimestamp(),clientUpdatedAt:new Date().toISOString()},{merge:false});status(`Licensing Budget saved · ${rows.length} projects · ${fmt(total())} USD`)}catch(error){console.error(error);status('Save failed: '+(error.code||error.message),true);alert('Licensing save failed: '+(error.message||error))}finally{button.disabled=false}
}
...
async function download(){
 ...
 const workbook=new ExcelJS.Workbook(),sheet=workbook.addWorksheet('Licensing Budget');sheet.addRow(['Department','Project Name','Unit Value (USD)']);rows.forEach(row=>sheet.addRow([row.department,row.projectName,row.unitValue===null?null:num(row.unitValue)]));sheet.addRow(['Total','',total()]);sheet.columns=[{width:36},{width:58},{width:22}];sheet.getRow(1).font={bold:true,color:{argb:'FFFFFFFF'}};sheet.getRow(1).fill={type:'pattern',pattern:'solid',fgColor:{argb:'FF0A2C61'}};sheet.getColumn(3).numFmt='#,##0;[Red]-#,##0';sheet.views=[{state:'frozen',ySplit:1}];const last=sheet.getRow(sheet.rowCount);last.font={bold:true,color:{argb:'FFFFFFFF'}};last.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FF082765'}};const buffer=await workbook.xlsx.writeBuffer(),blob=new Blob([buffer],{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download='Licensing_Budget_2027_USD.xlsx';document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000)
}
