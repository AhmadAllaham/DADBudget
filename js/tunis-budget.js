...
let rows=[],revision=0,ready=false,busy=false,dirty=false,started=false;
...
function controls(){for(const id of ['addRow','saveBudget'])$(id).disabled=!ready||busy;$('reloadBudget').disabled=!started||busy;document.querySelectorAll('#tunisBody input,#tunisBody button').forEach(el=>el.disabled=!ready||busy)}
...
function updateTotals(){
  ...
  const td=document.createElement('td');td.textContent=fmt(total(rows));tr.append(td,document.createElement('td'));footer.append(tr);
}
function render(){
  ...
  rows.forEach((row,i)=>{
    ...
    for(const key of ['code','description']){const input=document.createElement('input');input.value=row[key];input.maxLength=key==='code'?80:250;input.setAttribute('aria-label',`Line ${i+1} ${key}`);input.addEventListener('input',()=>{row[key]=input.value;changed()});tr.insertCell().append(input)}
    ...
    tr.insertCell().dataset.total='1';const remove=document.createElement('button');remove.type='button';remove.textContent='Remove';remove.className='remove-line';remove.setAttribute('aria-label',`Remove line ${i+1}`);remove.addEventListener('click',()=>{rows.splice(i,1);changed();render()});tr.insertCell().append(remove);
  });updateTotals();controls();
}
async function load(){
  if(busy)return;if(dirty&&!confirm('Discard unsaved changes and reload the saved budget?'))return;
  busy=true;controls();status('Loading saved Tunis budget…');
  try{const snapshot=await getDoc(ref()),data=snapshot.exists()?snapshot.data():null;const next=data?validateRows(data.rows):[];rows=next;revision=data?.revision||0;ready=true;dirty=false;render();status(data?`Loaded saved ${type.toUpperCase()} 2027 · revision ${revision}.`:'No saved budget yet. Add your first line.');}
...
async function save(){
  ...
  });rows=result.rows;revision=result.revision;dirty=false;render();status(`Saved ${type.toUpperCase()} 2027 successfully · revision ${revision}.`)}
...
$('addRow').addEventListener('click',()=>{if(rows.length>=500){status('Use at most 500 budget lines.',true);return}rows.push(blankRow());changed();render();$('tunisBody').lastElementChild?.querySelector('input')?.focus()});
