...
function mergeIT(department,allocation){if(!department||!allocation)return department;const out={...department,items:{...(department.items||{})}};Object.entries(IT_TARGETS).forEach(([source,targetName])=>{const value=allocation?.items?.[source]||{},target=key(targetName),found=Object.entries(out.items).find(([,item])=>key(item?.name)===target),accountCode=clean(value?.accountCode),match=found||(accountCode?[accountCode,{code:accountCode,name:targetName}]:null);if(!match)return;const[k,sourceItem]=match,current=out.items[k]||{code:clean(sourceItem?.code||k),name:clean(sourceItem?.name||targetName),budgetByMonth:{},actualByMonth:{},lyByMonth:{},fyBudget:0,actualUnperiodized:0,lyUnperiodized:0,hasLY:false},total=num(value?.total);out.items[k]={...current,newBudgetByMonth:annualMonths(total),itControlled:true,itAllocatedTotal:total}});return out}
