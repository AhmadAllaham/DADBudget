...
const workflowWeights={not_submitted:0,uploaded:25,submitted:45,under_review:70,returned:50,approved:100};
...
async function readAssigned(ids){
  ...
  await Promise.all(ids.map(async cc=>{
    const [ss,us]=await Promise.all([getDoc(doc(db,'budget_submission_status',cc)),getDoc(doc(db,'opex_budget_submissions',cc))]);
    if(ss.exists())statuses[cc]=ss.data();if(us.exists())uploads[cc]=us.data();
  }));
  return{statuses,uploads};
}
async function readAdmin(){
  const [statusSnap,uploadSnap]=await Promise.all([getDocs(collection(db,'budget_submission_status')),getDocs(collection(db,'opex_budget_submissions'))]),statuses={},uploads={};
  statusSnap.forEach(x=>statuses[x.id]=x.data());uploadSnap.forEach(x=>uploads[x.id]=x.data());return{statuses,uploads};
}
async function load(user){
  ...
  try{
    ...
    const progress=Math.round(total/ids.length);grid.innerHTML=cards;summary.textContent=`${ids.length} department(s) · Overall progress ${progress}% · Finance inputs: Salaries 15% + Depreciation 15%`;
    const kpi=[...document.querySelectorAll('.kpi-card')].find(x=>x.querySelector('span')?.textContent.trim()==='Budget Progress');if(kpi){kpi.querySelector('strong').textContent=progress+'%';kpi.querySelector('small').textContent=`${submittedCount} / ${ids.length} departments submitted · central inputs included`}
