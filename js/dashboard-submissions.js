...
async function load(user){
  ...
  try{
    ...
    const cards=ids.map(cc=>{
      const data=statuses[cc]||{},st=statusOf(data,!!uploads[cc]),name=data.departmentName||uploads[cc]?.name||profileNames[cc]||cc,w=departmentProgress(data,!!uploads[cc]);total+=w;
