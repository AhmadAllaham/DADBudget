...
async function readAssigned(ids){const statuses={},uploads={},baselineNames={};await Promise.all(ids.map(async cc=>{const [ss,us,bs]=await Promise.all([getDoc(doc(db,'budget_submission_status',cc)),getDoc(doc(db,'opex_budget_submissions',cc)),getDoc(doc(db,'opex_baseline_departments',cc))]);if(ss.exists())statuses[cc]=ss.data();if(us.exists())uploads[cc]=us.data();if(bs.exists())baselineNames[cc]=bs.data()?.name||''}));return{statuses,uploads,baselineNames}}
async function readAdmin(){const [statusSnap,uploadSnap,baselineSnap]=await Promise.all([getDocs(collection(db,'budget_submission_status')),getDocs(collection(db,'opex_budget_submissions')),getDocs(collection(db,'opex_baseline_departments'))]),statuses={},uploads={},baselineNames={};statusSnap.forEach(x=>statuses[x.id]=x.data());uploadSnap.forEach(x=>uploads[x.id]=x.data());baselineSnap.forEach(x=>baselineNames[x.id]=x.data()?.name||'');return{statuses,uploads,baselineNames}}
async function load(user){
  ...
  try{
    ...
    const assigned=Array.isArray(p.departments)?p.departments.map(String).filter(x=>x&&x!=='ALL'):[],profileNames=buildDepartmentNameMap(p);let statuses={},uploads={},baselineNames={},ids=[];
    if(admin){({statuses,uploads,baselineNames}=await readAdmin());ids=[...new Set([...Object.keys(baselineNames),...Object.keys(statuses),...Object.keys(uploads)])]}else{ids=[...new Set(assigned)];if(!ids.length){summary.textContent='No departments assigned.';return}({statuses,uploads,baselineNames}=await readAssigned(ids))}
    ...
    const cards=ids.map(cc=>{
      ...
      return `<div style="border:1px solid #d9e9e6;border-radius:10px;padding:12px 14px;background:#fbfdfd"><div style="display:none"></div><div style="font-size:12px;font-weight:1000;color:#173f46;margin-top:0">${name}</div><div style="margin-top:8px;display:flex;justify-content:space-between;gap:8px;align-items:center"><span style="font-size:9px;font-weight:1000;color:${st==='manager_returned'?'#a63b3b':'#087a64'}">${labels[st]||st}</span><span style="font-size:9px;color:#173f46;font-weight:1000">${w}%</span></div><div style="height:7px;border-radius:999px;background:#e7efee;margin-top:6px;overflow:hidden"><div style="height:100%;width:${w}%;background:#12a397"></div></div><div style="display:flex;gap:5px;flex-wrap:wrap;margin-top:8px"><span style="display:inline-flex;padding:4px 6px;border-radius:999px;background:#eef7ff;color:#225c8e;font-size:8px;font-weight:1000">Workflow: ${workflowContribution}/${WORKFLOW_SHARE}</span>${inputTag('Salaries',data.salariesReady===true)}${inputTag('Depreciation',data.depreciationReady===true)}</div>${managerInfo}${managerNote}${data.note?`<div style="font-size:8px;color:#7b8f92;margin-top:6px">Finance note: ${data.note}</div>`:''}${upload.fileName?`<div style="font-size:8px;color:#7b8f92;margin-top:4px">Latest upload: ${upload.fileName}${upload.revision?` · R${upload.revision}`:''}</div>`:''}</div>`;
    }).join('');
