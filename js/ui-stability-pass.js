...
function normalizeSubscriptionVariance(){
 ...
 try{[...body.rows].forEach(tr=>{if(tr.querySelector('.empty-state')||tr.cells.length<19)return;const budget=numberFromCell(tr.cells[7]),actual=numberFromCell(tr.cells[8]),delta=actual-budget,pct=Math.abs(budget)<.005?'—':`${(delta/Math.abs(budget)*100).toFixed(1)}%`;tr.cells[9].textContent=displayMoney(delta);tr.cells[10].textContent=pct;[tr.cells[9],tr.cells[10]].forEach(td=>{td.classList.remove('good','bad');td.classList.add(delta>0?'bad':'good')})})}finally{subscriptionVarianceSync=false}
}
