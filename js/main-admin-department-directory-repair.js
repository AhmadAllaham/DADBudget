(function(){
'use strict';
const MAIN='PST3chwdZmaQGeG25t4ym9Vlixe2',DIR_DOC='department_directory_fy2027',SESSION_KEY='dadBudgetDirectoryRepairV1';
...
async function repair(user,profile={}){
  if(!user||user.uid!==MAIN||sessionStorage.getItem(SESSION_KEY)==='1')return;
  ...
  await Promise.all([
    f.setDoc(f.doc(api.db,'system_status',DIR_DOC),{
      fiscalYear:2027,departmentCount:directory.length,directory,source:'Canonical Finance OPEX baseline',updatedBy:user.uid,updatedByEmail:clean(user.email||profile.email).toLowerCase(),updatedAt:f.serverTimestamp(),clientUpdatedAt:new Date().toISOString()
    },{merge:false}),
    f.setDoc(f.doc(api.db,'opex_baseline_meta','current'),{
      departments:ids,departmentDirectory:directory,departmentCount:directory.length,directoryUpdatedAt:f.serverTimestamp()
    },{merge:true})
  ]);
