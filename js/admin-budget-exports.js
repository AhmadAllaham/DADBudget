...
async function exportCapex(btn){
  ...
  try{
    ...
    const paymentSheet=wb.addWorksheet('Payment Schedule');paymentSheet.addRow(['Fund Center','Department','Asset Category','Budget Description','FY CAPEX 2027',...MONTHS,'Q1 Payments','Q2 Payments','Q3 Payments','Q4 Payments','Total Payments','Remaining','Workflow Status','Revision','Source File']);departments.forEach(d=>d.lines.forEach(line=>{const months=Array(12).fill(0);d.payments.filter(p=>clean(p.requestId)===clean(line.requestId)).forEach(p=>{const month=Number(clean(p.expectedPaymentDate).slice(5,7))-1;if(month>=0&&month<12)months[month]+=num(p.amount)});const quarters=[months.slice(0,3),months.slice(3,6),months.slice(6,9),months.slice(9,12)].map(a=>a.reduce((s,v)=>s+v,0)),paid=months.reduce((s,v)=>s+v,0);paymentSheet.addRow([d.cc,d.name,clean(line.category),clean(line.description),num(line.total),...months,...quarters,paid,num(line.total)-paid,workflow(d.meta),num(d.meta.revision),clean(d.meta.fileName)])}));styleSheet(paymentSheet,[18,32,28,42,18,...Array(12).fill(13),16,16,16,16,18,18,20,11,38]);moneyColumns(paymentSheet,5,23);
