...
async function exportCapex(btn){
  ...
  try{
    ...
    const paymentSheet=wb.addWorksheet('Payment Schedule');paymentSheet.addRow(['Request ID','Fund Center','Department','Payment No.','Payment Type','Expected Payment Date','Amount','Status','Notes','Workflow Status','Revision','Source File']);departments.forEach(d=>d.payments.forEach(p=>paymentSheet.addRow([clean(p.requestId),d.cc,d.name,clean(p.paymentNo),clean(p.paymentType),clean(p.expectedPaymentDate),num(p.amount),clean(p.status)||'Planned',clean(p.notes),workflow(d.meta),num(d.meta.revision),clean(d.meta.fileName)])));styleSheet(paymentSheet,[22,18,32,14,22,23,18,16,38,20,11,38]);moneyColumns(paymentSheet,7,7);
