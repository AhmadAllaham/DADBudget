...
async function exportOpex(btn){
  ...
  try{
    ...
    for(const cc of orderedIds){
      const baselineMeta=baselineMap.get(cc)||{},submissionMeta=submissionMap.get(cc)||{},baseline=baselineMeta.payload?await decodeSubmission(baselineMeta,cc):{cc,name:baselineMeta.name,items:{}},submitted=submissionMeta.payload?await decodeSubmission(submissionMeta,cc):null,data=mergeOpexDepartment(baseline,submitted),items=data.items||{},seen=new Set(),orderedItems=[];
...
async function exportCapex(btn){
  ...
  try{
    ...
    const departments=orderedIds.map(cc=>{const meta=capexMap.get(cc)||{},raw=Array.isArray(meta.rows)?meta.rows:[],lines=raw.map(row=>{const months=MONTHS.map((_,i)=>num(row?.months?.[i])),total=months.reduce((sum,value)=>sum+value,0);return{...row,months,total}});return{cc,name:clean(meta.departmentName||lines[0]?.department||nameMap.get(cc)||cc),lines,total:lines.reduce((sum,line)=>sum+line.total,0),meta}});
    if(!departments.length)throw new Error('No CAPEX departments were found.');
    const wb=new ExcelJS.Workbook();wb.creator='DAD Budget 2027';wb.created=new Date();
    const summary=wb.addWorksheet('Summary');summary.addRow(['Fund Center','Department','CAPEX Requests','FY CAPEX 2027','Status','Revision','Submitted By','Submitted At','Source File']);departments.forEach(d=>summary.addRow([d.cc,d.name,d.lines.length,d.total,workflow(d.meta),num(d.meta.revision),clean(d.meta.submittedEmail),dateText(d.meta.submittedAt)||clean(d.meta.clientSubmittedAt),clean(d.meta.fileName)]));styleSheet(summary,[18,34,16,20,20,11,28,22,38]);moneyColumns(summary,4,4);
    const headers=['Company','Fund Center','Department','Asset Category','Budget Description','Justification','Priority','Quantity',...MONTHS,'Q1 2027','Q2 2027','Q3 2027','Q4 2027','FY CAPEX 2027','Status','Revision','Submitted By','Source File'];
    const widths=[18,18,32,28,42,42,14,12,...Array(12).fill(13),16,16,16,16,20,20,11,28,38];
    const all=wb.addWorksheet('All CAPEX');all.addRow(headers);
    departments.forEach(d=>{
      const sourceLines=d.lines.length?d.lines:[{months:Array(12).fill(0),total:0}],rows=sourceLines.map(line=>{const q1=line.months.slice(0,3).reduce((s,v)=>s+v,0),q2=line.months.slice(3,6).reduce((s,v)=>s+v,0),q3=line.months.slice(6,9).reduce((s,v)=>s+v,0),q4=line.months.slice(9,12).reduce((s,v)=>s+v,0);return[clean(line.company),d.cc,d.name,clean(line.category),clean(line.description),clean(line.justification),clean(line.priority),num(line.quantity),...line.months,q1,q2,q3,q4,line.total,workflow(d.meta),num(d.meta.revision),clean(d.meta.submittedEmail),clean(d.meta.fileName)]});
      rows.forEach(row=>all.addRow(row));
    });
    styleSheet(all,widths);moneyColumns(all,9,25);
