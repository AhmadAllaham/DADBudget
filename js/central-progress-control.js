...
async function stateFor(cc,module){const key=`${module}:${cc}`,shared=window.DADSubmissionControlState?.[key];if(shared){const d={...shared};cache.set(key,d);return d}if(cache.has(key))return cache.get(key);const collectionName=module==='CAPEX'?'capex_budget_submissions':'opex_budget_submissions',[s,u]=await Promise.all([getDoc(doc(db,'budget_submission_status',cc)),getDoc(doc(db,collectionName,cc))]),d={...(s.exists()?s.data():{}),...(u.exists()?u.data():{})};cache.set(key,d);return d}
