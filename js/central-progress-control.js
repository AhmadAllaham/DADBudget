...
async function enhanceRow(tr){
  ...
  try{const st=await stateFor(cc),cells=[...tr.children],currentStatus=cells.find(x=>x.querySelector?.('.badge')),note=cells.find(x=>x.querySelector?.('[data-note]'));
...
start();
import('./finance-return-button.js?v=20260817-return-direct-2').catch(e=>console.error('Finance return button failed:',e));