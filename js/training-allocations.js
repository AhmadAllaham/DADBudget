...
async function load(){
 ...
 try{
  ...
  const snapshot=await fs.getDocs(fs.collection(api.db,'system_status'));snapshot.docs.filter(doc=>clean(doc.id).startsWith('training_allocation_')).forEach(doc=>allocations.set(clean(doc.id).replace(/^training_allocation_/,''),doc.data()||{}));
