...
const MAIN_ADMIN_UID='PST3chwdZmaQGeG25t4ym9Vlixe2',DOC_PREFIX='subscription_budget_';
...
let directory=[],profile=null,observer=null,bodyObserver=null,busy=false,admin=false,hideZero=true,allRows=null,loadingAll=false;
...
function selectedLabel(select){return clean(select?.selectedOptions?.[0]?.textContent)||'Select Department'}
function rebuildSearch(){
  const select=document.getElementById('subscriptionDept'),input=document.getElementById('subscriptionDeptQuickSearch'),list=document.getElementById('subscriptionDeptSearchList');
  if(!select||!input||!list)return;
  list.innerHTML='';
  [...select.options].forEach(o=>{const x=document.createElement('option');x.value=o.textContent;x.dataset.value=o.value;list.appendChild(x)});
  input.value=selectedLabel(select);
}
function chooseSearch(){
  const select=document.getElementById('subscriptionDept'),input=document.getElementById('subscriptionDeptQuickSearch');
  if(!select||!input)return;
  const q=clean(input.value).toLowerCase(),options=[...select.options];
  ...
  if(!match){input.value=selectedLabel(select);return}
...
function ensureSearchUi(){
  const select=document.getElementById('subscriptionDept'),filter=select?.closest('.sub-filter');if(!select||!filter)return;
  let input=document.getElementById('subscriptionDeptQuickSearch'),list=document.getElementById('subscriptionDeptSearchList');
  if(!input){
    input=document.createElement('input');input.id='subscriptionDeptQuickSearch';input.type='search';input.placeholder='Search department or Fund Center...';input.setAttribute('list','subscriptionDeptSearchList');input.autocomplete='off';
    list=document.createElement('datalist');list.id='subscriptionDeptSearchList';
    select.insertAdjacentElement('beforebegin',input);document.body.appendChild(list);select.style.display='none';
    input.addEventListener('change',chooseSearch);input.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();chooseSearch()}});
    select.addEventListener('change',()=>setTimeout(()=>{input.value=selectedLabel(select)},0));
    filter.style.gridTemplateColumns='minmax(330px,470px) minmax(250px,1fr) auto';
  }
  rebuildSearch();
}
function apply(){
  const select=document.getElementById('subscriptionDept');if(!select||busy)return;
  busy=true;observer?.disconnect();
  try{
    const previous=clean(select.value),allowed=new Set(directory.map(d=>d.cc));
    if(admin){
      let all=[...select.options].find(o=>o.value==='ALL');
      if(!all){all=document.createElement('option');all.value='ALL';all.textContent='All Departments';select.appendChild(all)}else all.textContent='All Departments';
      const current=[...select.options],known=new Set(current.map(o=>o.value));
      directory.forEach(d=>{
        if(known.has(d.cc)){const existing=current.find(o=>o.value===d.cc);if(existing)existing.textContent=`${d.cc} · ${d.name}`;return}
        const o=document.createElement('option');o.value=d.cc;o.textContent=`${d.cc} · ${d.name}`;select.appendChild(o);
      });
    }else{
      [...select.options].forEach(o=>{if(!allowed.has(clean(o.value)))o.remove()});
      directory.forEach(d=>{let o=[...select.options].find(x=>clean(x.value)===d.cc);if(!o){o=document.createElement('option');o.value=d.cc;select.appendChild(o)}o.textContent=`${d.cc} · ${d.name}`});
    }
    const options=[...select.options].sort((a,b)=>{
      if(a.value==='ALL')return -1;if(b.value==='ALL')return 1;
      const an=clean(a.textContent).replace(/^\d+\s*·\s*/,''),bn=clean(b.textContent).replace(/^\d+\s*·\s*/,'');
    ...
    ensureSearchUi();
    if(changed&&typeof select.onchange==='function'&&select.value!=='ALL')setTimeout(()=>select.dispatchEvent(new Event('change',{bubbles:true})),0);
  }finally{
    busy=false;if(observer&&document.body.contains(select))observer.observe(select,{childList:true,subtree:true});
  }
}
function zeroRow(tr){
  if(!tr||tr.classList.contains('total-row')||tr.querySelector('.empty-state'))return false;
  const cells=[...tr.cells].slice(5);
  if(!cells.length)return true;
  return cells.every(td=>{
    let s=clean(td.textContent).replace(/,/g,'').replace(/%/g,'');
    if(!s||s==='—'||s==='-')return true;
    if(/^\(.*\)$/.test(s))s='-'+s.slice(1,-1);
    const n=Number(s);return !Number.isFinite(n)||Math.abs(n)<.005;
  });
}
function applyZeroRows(){
  ...
  const btn=document.getElementById('subscriptionZeroToggle');if(btn){btn.textContent=hideZero?'Show Zero':'Hide Zero';btn.classList.toggle('active',hideZero)}
}
function ensureZeroUi(){
  const filter=document.querySelector('.sub-filter');if(!filter)return;
  let btn=document.getElementById('subscriptionZeroToggle');
  if(!btn){
    btn=document.createElement('button');btn.id='subscriptionZeroToggle';btn.type='button';btn.className='sub-btn ghost';btn.style.height='40px';btn.style.whiteSpace='nowrap';filter.appendChild(btn);
    btn.onclick=()=>{hideZero=!hideZero;try{localStorage.setItem('dadBudgetSubscriptionsHideZero',hideZero?'1':'0')}catch(_){}if(clean(document.getElementById('subscriptionDept')?.value)==='ALL')renderAll();else applyZeroRows()};
  }
...
async function loadAll(force=false){
  ...
  if(status){status.textContent='Loading all departments...';status.classList.remove('error');status.classList.remove('ready')}
  ...
  try{
    ...
    renderAll();
    document.getElementById('subscriptionDownload')?.setAttribute('disabled','disabled');
    document.getElementById('subscriptionUpload')?.setAttribute('disabled','disabled');
...
function renderAll(){
  ...
  ['kpiBudget','kpiActual','kpiFy26','kpiFy27'].forEach((id,i)=>{const el=document.getElementById(id),vals=[t.budgetYtd,t.actualYtd,t.fy26,t.fy27];if(el)el.textContent=money(vals[i])});
...
function bindEnhancements(){
  const select=document.getElementById('subscriptionDept');if(!select)return false;
  if(!observer){observer=new MutationObserver(()=>{if(!busy)setTimeout(apply,0)});observer.observe(select,{childList:true,subtree:true})}
  apply();ensureSearchUi();ensureZeroUi();
  if(select.dataset.subscriptionEnhanced!=='1'){
    select.dataset.subscriptionEnhanced='1';
    select.addEventListener('change',e=>{
      const value=clean(select.value);document.getElementById('subscriptionDeptQuickSearch')&&(document.getElementById('subscriptionDeptQuickSearch').value=selectedLabel(select));
      ...
      allRows=null;setTimeout(applyZeroRows,120);
    },true);
    const search=document.getElementById('subscriptionSearch');search?.addEventListener('input',e=>{if(clean(select.value)==='ALL'&&admin){e.stopImmediatePropagation();renderAll()}},true);
  ...
  if(body&&!bodyObserver){bodyObserver=new MutationObserver(()=>{if(clean(select.value)!=='ALL')applyZeroRows()});bodyObserver.observe(body,{childList:true,subtree:true})}
  return true;
}
async function boot(user){
  if(!user)return;profile=await loadProfile(user);admin=isAdmin(user,profile);await loadDirectory();
  let tries=0;const timer=setInterval(()=>{tries++;if(bindEnhancements()||tries>80)clearInterval(timer)},100);
  window.addEventListener('load',()=>setTimeout(()=>{apply();ensureSearchUi();ensureZeroUi()},250));
}
onAuthStateChanged(auth,user=>boot(user).catch(e=>console.warn('Subscriptions filter enhancements failed',e)));
